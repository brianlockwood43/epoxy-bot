...
class MemoryReviewModeAllTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def _remember(self, *, source_path: str, owner_override_active: bool) -> dict | None:
    ...
    async def test_all_defaults_to_candidate_for_all_sources(self):
        for source_path in ("manual_remember", "manual_profile", "auto_capture", "mining"):
            saved = await self._remember(source_path=source_path, owner_override_active=False)
            self.assertIsNotNone(saved)
            self.assertEqual(saved["lifecycle"], "candidate")

    async def test_all_supports_owner_override_for_manual_remember_only(self):
        remember_saved = await self._remember(source_path="manual_remember", owner_override_active=True)
        self.assertIsNotNone(remember_saved)
        self.assertEqual(remember_saved["lifecycle"], "active")

        profile_saved = await self._remember(source_path="manual_profile", owner_override_active=True)
        self.assertIsNotNone(profile_saved)
        self.assertEqual(profile_saved["lifecycle"], "candidate")
